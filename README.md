...
RecordFilterWord=http|www ;保存消息到数据库的过滤词，使用 | 分割
...
AnyYearCloudOrder=^(20\d{2})年词云$ ;绘制任意年份的词云，此指令只会进行正则模式处理
AnyMonthCloudOrder=^(20\d{2})-(\d{1,2})月词云$
PersonalAnyYearCloudOrder=^我的(20\d{2})年词云$
PersonalAnyMonthCloudOrder=^我的(20\d{2})-(\d{1,2})月词云$
...
CyclePersonText=今天共有<personnum>位群友产生<sentensenum>条发言，前三的群友是:\n<content>
