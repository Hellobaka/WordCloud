...
{
    public class DrawPersonalLastYearCloud : IOrderModel
    {
        public bool ImplementFlag { get; set; } = true;

        public string GetOrderStr() => CloudConfig.PersonalLastYearOrder;
        ...
        public FunctionResult Progress(CQGroupMessageEventArgs e)//群聊处理
        {
            ...
            var drawResult = DrawWordCloud.Draw(e.FromGroup, dt, dt.AddYears(1), e.FromQQ);
            string statistics = $"{e.FromQQ.CQCode_At()}统计时段: {dt.ToString("G").Replace(" 0:00:00", "")} - {dt.AddYears(1).ToString("G").Replace(" 0:00:00", "")}，共计: {drawResult.WordNum}个词汇";
